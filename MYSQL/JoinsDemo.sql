...
SELECT productCode,productName,textDescription FROM products p  -- with alias
	INNER JOIN productlines p1
		ON p.productLine=p1.productLine;

SELECT productCode,productName,textDescription FROM products p -- without alias
	INNER JOIN productlines 
		ON p.productLine=productlines.productLine;		
...
SELECT employeeNumber, lastName, firstName,officeCode FROM employees -- outer Query
	WHERE officeCode
		IN (SELECT officeCode FROM offices WHERE country='USA'); -- INNER QUERY ACTS AS CRITERIA
